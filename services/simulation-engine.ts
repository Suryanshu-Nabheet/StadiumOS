...
function mutateZones(zones: ZoneMetrics[]): ZoneMetrics[] {
  return zones.map((z) => {
    ...
    return {
      ...
      throughput: Math.round(clamp(z.throughput + randomBetween(-80, 120), 100, 1200)),
      trend: delta > 2 ? "up" : delta < -2 ? "down" : "stable",
    };
  });
}

function mutateHeatmap(snapshot: StadiumSnapshot): StadiumSnapshot["heatmap"] {
...
function maybeAddEmergency(
  ...
  const newIncident: EmergencyIncident = {
    ...
    summary: "Autonomous emergency agent flagged anomaly from live sensor fusion.",
    coordinates: {
...
export function tickSimulation(
  ...
  const occupancy = Math.round(
    clamp(snapshot.occupancy + randomBetween(-400, 600), 90000, 132000),
  );

  const updatedSnapshot: StadiumSnapshot = {
    ...snapshot,
    ...
    occupancy,
    occupancyPercent: (occupancy / 132000) * 100,
    crowdStressScore: clamp(
    ...
    kpis: snapshot.kpis.map((kpi) => {
      if (kpi.label === "Live Occupancy") {
        return { ...kpi, value: occupancy };
      }
      if (kpi.label === "Crowd Stress") {
        return { ...kpi, value: Math.round(snapshot.crowdStressScore) };
      }
      return kpi;
    }),
  };
  ...
  const updatedEmergencies = emergencies.map((inc) => {
    ...
    if (inc.status === "dispatching" && Math.random() > 0.6) {
      return { ...inc, status: "responding" as const, etaMinutes: Math.max(1, inc.etaMinutes - 1) };
    }
    return inc;
  });
  ...
  return {
    ...
    emergencies: maybeAddEmergency(updatedEmergencies),
  };
}
